...
import com.github.zillionchu.zillionchucompanybiz.thirdparty.two.feign.FinancinginfodtoFeign;
 ...
 */
@Component
public class FinancinginfodtoHttpPost implements HttpStrategy<Financinginfodto, Map>, LoggerBase {

    @Autowired
    FinancinginfodtoFeign demoFeign;

    private static final String H_HYSTRIXCOMMAND_GROUPKEY = "FinancinginfodtoHttpPost";


    @Override
    public Map httpRequest(Financinginfodto demoEntity) {
        return new FinancinginfodtoPostHystrixCommand(demoFeign).execute();
    }


    static class FinancinginfodtoPostHystrixCommand extends HystrixCommand<Map> implements LoggerBase {

        private FinancinginfodtoFeign demoFeign;

        protected FinancinginfodtoPostHystrixCommand(FinancinginfodtoFeign demoFeign) {
            super(setter());
        ...
        public static Setter setter() {
            ...
            // 服务标识
            HystrixCommandKey commandKey = HystrixCommandKey.Factory.asKey("FinancinginfodtoBiz");
            // 线程池名称
            HystrixThreadPoolKey threadPoolKey = HystrixThreadPoolKey.Factory.asKey("FinancinginfodtoBiz-pool");
