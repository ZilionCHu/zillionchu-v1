...
import com.github.zillionchu.zillionchucompanybiz.thirdparty.two.feign.VideointerfacedtoFeign;
 ...
 */
@Component
public class VideointerfacedtoHttpPost implements HttpStrategy<Videointerfacedto, Map>, LoggerBase {

    @Autowired
    VideointerfacedtoFeign demoFeign;

    private static final String H_HYSTRIXCOMMAND_GROUPKEY = "VideointerfacedtoHttpPost";


    @Override
    public Map httpRequest(Videointerfacedto demoEntity) {
        return new VideointerfacedtoPostHystrixCommand(demoFeign).execute();
    }


    static class VideointerfacedtoPostHystrixCommand extends HystrixCommand<Map> implements LoggerBase {

        private VideointerfacedtoFeign demoFeign;

        protected VideointerfacedtoPostHystrixCommand(VideointerfacedtoFeign demoFeign) {
            super(setter());
        ...
        public static Setter setter() {
            ...
            // 服务标识
            HystrixCommandKey commandKey = HystrixCommandKey.Factory.asKey("VideointerfacedtoBiz");
            // 线程池名称
            HystrixThreadPoolKey threadPoolKey = HystrixThreadPoolKey.Factory.asKey("VideointerfacedtoBiz-pool");
